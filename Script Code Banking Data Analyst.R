...
banking_data %>%
  ...
  labs(
    ...
    subtitle="This chart illustrates the distribution of Gender in banking dataset(N=100,000)",
    x="Gender",
    y="Percentage(%)"
  )+
  theme_minimal()+
...
banking_data %>%
  ...
  labs(
    ...
    subtitle="Performance the relationship between monthly transactions and balance amount (N = 100,000)",
...
banking_data %>%
  count(loan_status)%>%
  ggplot(mapping=aes(x=loan_status,y=n,fill=loan_status))+
  geom_col(
    width=0.3
  )+
  geom_text(
...
banking_data %>%
  ...
  geom_histogram(
    ...
    labs(
      ...
      subtitle="Histogram illustrates the distribution of credit score",
      x='Credit Score',
      y='Number of Customers'
    )+
  theme_minimal()+
